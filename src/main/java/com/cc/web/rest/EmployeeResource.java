...
/**
 * REST controller for managing {@link com.cc.domain.Employee}.
 */
@RestController
@RequestMapping("/api")
public class EmployeeResource {
    ...
    @PostMapping("/employees/forms")
    public ResponseEntity<EmployeeDTO> createEmployeeByFroms(@Valid @RequestBody Employee employee) throws URISyntaxException {
        log.debug("REST request to save Employee : {}", employee);
        if (employee.getId() != null) {
            throw new BadRequestAlertException("A new employee cannot already have an ID", ENTITY_NAME, "idexists");
        }
        EmployeeDTO result = employeeService.save(employee);
    ...
    public ResponseEntity<List<EmployeeDTO>> getAllEmployees(EmployeeCriteria criteria, Pageable pageable, @RequestParam(required = false) MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
